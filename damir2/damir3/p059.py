...
class Cipher():
  def __init__(self,
    ...
    if not isinstance(minimum_match_percentage, int) \
       ...
       or not isinstance(minimum_length_words, int) \
    ...
    self.__cipher = ''
    self.__english_dictionary = self.__load_file('p059_words_alpha.txt')
  ...
  def decode_data(self, data: str) -> str:
    ...
    for key in product([c for c in self.__password_letters], repeat=3):
      output = ''.join(chr(int(x) ^ ord(y)) for (x, y) in zip(data.split(','), cycle(key)))
      all_words = list(re.findall(r'\b(\w{' + str(self.__minimum_length_words) + r',})\b', output))
      potential_words = list(re.findall(r'(?:(?=^)|(?<=[,.\s]))([a-zA-Z]{' + str(self.__minimum_length_words) + r',})(?=\b)', output))
  ...
  @staticmethod
  def sum_ascii_codes(data: str) -> int:
    if not isinstance(data, str):
      raise TypeError('Expecting a non-empty string.')
    if len(data) == 0:
      raise TypeError('Expecting a non-empty string.')
  ...
  def euler_solution(self):
    return self.decode_file_message('p059_cipher.txt')
