...
class Test_p12(unittest.TestCase):
  def test_prime_factors(self):
    ...
    _prime_factors = _prime_calc.calc_prime_factors(28)
    self.assertEqual(_prime_factors, [2, 2, 7])

  def test_combined_prime_factors(self):
    _prime_calc = PrimeCalc()
    self.assertEqual(_prime_calc.combine_prime_factors([2, 2, 7]), [1, 2, 4, 7, 14, 28])
  ...
  def test_type_errors(self):
    _prime_calc = PrimeCalc()
    self.assertRaises(TypeError, _prime_calc.combine_prime_factors, 100)
    self.assertRaises(TypeError, _prime_calc.combine_prime_factors, [2, 2, '7'])
    self.assertRaises(TypeError, _prime_calc.calc_prime_factors, [2, 2, '7'])
    self.assertRaises(TypeError, _prime_calc.calc_prime_factors, '7')
